...
function getLocalIdForGuid(guid) {
  // TODO: deleted items needs to be addressed
...
function getGuidForLocalId(localId) {
  //L.log("getGuidForLocalId", idToGuidMap[localId]);
...
function setGuidForLocalId(guid, localId) {
  ...
  then(function() {
    ...
    guidToIdMap[guid] = localId;
    idToGuidMap[localId] = guid;
    //L.log("updated map for values", guid, localId);
    return;
  });
}
...
module.exports = {
  ...
  specialIdForGUID: kSpecialIds.specialIdForGUID.bind(kSpecialIds),
  cacheGuidForLocalId: cacheGuidForLocalId,
  specialGUIDs: { tags: TAGS, mobile: MOBILE, menu: MENU },
