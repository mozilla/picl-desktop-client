...
BookmarksItem.prototype.getPositionForChildId = function(childId) {
  ...
  let position = this.children.indexOf(childId);
  if (position === -1) {
    throw new Error("BookmarksItem.getPositionForChildId can't find childId "+childId+" in parent with id "+this.id);
  }
  return position;
}
