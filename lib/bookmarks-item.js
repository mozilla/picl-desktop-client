...
BookmarksItem.prototype.toJSON = function toJSON() {
  ...
  if (this.isQuery()) {
    ...
    result.folderName = this.title; // TODO: not sure what folderName should be
  }
  if (this.isSeparator()) {
  ...
  if (this.tags) result.tags = this.tags;
    ...
    // Don't use this for now, let's just use the browser API to get the list of tags
...
function createBookmarkAsync(bookmarksItem, localParentId) {
  ...
  let parent = BookmarksStore.getItemForId(bookmarksItem.parentid);
  if (!parent) {
    L.log("BookmarksItem.createBookmarkAsync: can't find parent for bookmarksItem", bookmarksItem);
    return resolve(null);
  }
  let position = BookmarksStore.getItemForId(bookmarksItem.parentid).getPositionForChildId(bookmarksItem.id);
