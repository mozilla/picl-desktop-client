...
const SYNCED_COLLECTIONS = [ PASSWORDS_COLLECTION_NAME, BOOKMARKS_COLLECTION_NAME, TABS_COLLECTION_NAME, HISTORY_COLLECTION_NAME ];
...
SyncMediator.prototype.pullHistory = function() {
  L.log("Pulling history from server");
  return this.ssClient.readCollection({ collection: HISTORY_COLLECTION_NAME }).
  then(function (result) {
    HistoryAdapter.stopTracking();
    return HistoryAdapter.update(result.items).
    then(function () {
      HistoryAdapter.startTracking();
      return result;
    });
  }).
  then(null, function (err) {
    if (err.code === 404) L.log("Collection not found", HISTORY_COLLECTION_NAME);
    else L.log("Pull history error", err, err.stack);
    return null;
  });
};
...
SyncMediator.prototype.pushHistory = function() {
  var self = this;
  return HistoryAdapter.read().
  then(function (localHistoryItems) {
    if (localHistoryItems.length > 0) {
      L.log("Pushing local history items");
      return self.ssClient.updateCollection({ collection: HISTORY_COLLECTION_NAME, items: localHistoryItems }).
      then(function (result) {
        L.log("Push history success", result);
        return result.version;
      });
    }
    else {
      L.log("Local history empty");
      return resolve(null);
    }
  }).
  then(null, function(err) {
    L.log("Push history error", err.message, err.stack);
    throw err;
  });
};
