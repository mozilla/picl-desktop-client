...
function processReadBookmarksQueryRow(row, results) {
  ...
  if (!oneResult) {
    ...
    oneResult.localParent = row.getResultByName('b_parent');
...
// returns promise for all local bookmark items
function readLocalBookmarks() {
  ...
  then(function (results) { // resolve local parent ids -> parent guids
    return group(Object.keys(results).map(function (guid) {
      ...
      return BookmarksGuidService.getGuidForLocalId(itemInfo.localParent).
      then(function (parentGuid) {
        itemInfo.parent = parentGuid;
        return BookmarksItem(itemInfo);
      });
    })).
    then(function (bookmarkItems) {
      return bookmarkItems.map(function (item) { return item.toJSON(); });
    });
  }).then(null, function(error) {
    L.log("readLocalBookmarks error", error.message);
  });
}
...
function createItem(bookmarksItem) {
  BookmarksGuidService.getLocalIdForGuid(bookmarksItem.parent).
  then(function (localParentId) {
    L.log("creating item for parent", localParentId, "info", bookmarksItem);
  ...
  then(function (localId) {
    L.log("done creating item, about to set guid for local id", localId, bookmarksItem.id);
    // After creating item, update its local guid to match the
    // remote guid
    return BookmarksGuidService.setGuidForLocalId(bookmarksItem.id, localId).
...
function updateItems(bookmarksItems) {
  ...
  bookmarksItems.forEach(function (item) {
    ...
    // This creates entries for the parents that need their ids fetched that aren't in the
    // map explicitly
    if (!(item.parent in itemMap) && !BookmarksGuidService.isSpecialGUID(item.parent)) {
      itemMap[item.parent] = null;
    }
  });
  var itemGuids = Object.keys(itemMap);
  return BookmarksGuidService.getLocalIdsForGuids(itemGuids).
  then(function (existingItemGuidToIdMap) {
    L.log("existing ItemGUid to Id map", existingItemGuidToIdMap);
    var processGuid = function(guid) {
      if (guid in existingItemGuidToIdMap) {
