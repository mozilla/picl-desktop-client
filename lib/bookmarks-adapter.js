...
function createItem(bookmarksItem) {
  ...
  if (bookmarksItem.isFolder()) {
    ...
    then(function () {
      ...
      if (bookmarksItem.description) {
        PlacesUtils.annotations.setItemAnnotation(
          newLocalId, DESCRIPTION_ANNO, bookmarksItem.description, 0,
          PlacesUtils.annotations.EXPIRE_NEVER);
      }
      return setItemIdForGuid(newLocalId, bookmarksItem.id).
      then(function (localId) {
        L.log("create item", bookmarksItem, localParentId, newLocalId);
        return null;
      });
    });
  }
...
function updateLocalBookmarks(bookmarksItems) {
  var sortedFolders = topoSortFolders(bookmarksItems.filter(function (item) { return item.isFolder(); }));
  updateItems(sortedFolders).
  then(function () {
  });
  return;
  var uri;
