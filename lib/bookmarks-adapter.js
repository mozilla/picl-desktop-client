...
function readLocalBookmarks() {
  ...
  then(function (results) { // resolve local parent ids -> parent guids
    ...
    then(function (bookmarkItems) {
      // filter out tag items and add in any deleted items the adapter has seen deleted
      // via the bookmarks observer
...
var bookmarksObserver = {
  ...
  onBeforeItemRemoved: function(aItemId, aItemType, aParentId, aGUID, aParentGUID) {
    //L.log("Before item removed", aItemId, aItemType, aParentId, aGUID, aParentGUID);
  },
  onItemRemoved: function(itemId, parentId, index, type, uri, guid, parentGuid) {
    if (tracking) {
      dirty = true;
      // Need to fetch the actual guid from our service, because some Firefox
      // caching is causing the value provided by this observer to be incorrect.
      // When we create an item in Places, then it is assigned a guid. We re-write
      // that GUID directly with SQL but the caching layer is screwing us somehow here.
      BookmarksGuidService.getGuidForLocalId(itemId).
      then(function (guid) {
        // Need to check if our guid service has previously recorded a guid for this id.
        // If guid is null, then it's probably something we want to ignore.
        if (guid) {
