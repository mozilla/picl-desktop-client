...
const DESCRIPTION_ANNO     = require('bookmarks-item').DESCRIPTION_ANNO;
const SIDEBAR_ANNO         = require('bookmarks-item').SIDEBAR_ANNO;
const QUERY_ANNO           = require('bookmarks-item').QUERY_ANNO;

// TODO:
//  - BookmarksGUIDService that tracks and caches guids, manages special guids
//  - Consider a getActiveRecord() (or similar) on BookmarksItem to manage interfacing
//      with Places on behalf of this item
//  - Think about data models more. Is BookmarksItem an abstract data model or should it
//      be an active record?
...
// results should be in the form of { itemMap: {}, idToGUIDMap: {}, tagMap: {} }
function processReadBookmarksQueryRow(row, results) {
  var oneResult,
      localId = row.getResultByName('b_id'),
      // Use BookmarksGUIDService.getGUIDForId to try to resolve guid first, because it
      // will canonicalize the guids for the special folders for us.
      guid = BookmarksGUIDService.getGUIDForLocalId(localId) || row.getResultByName('b_guid');
  ...
  if (!BookmarksGUIDService.isSpecialGUID(guid)) {
    ...
    if (false && oneResult.parent === kSpecialIds.tags) { // TODO fix this
...
// returns promise for all raw bookmark info results
function readRawAllBookmarks() {
  var stmt = PlacesAdapter.createAsyncStatement(BOOKMARK_QUERY);
  var results = { itemMap: {}, idToGUIDMap: {}, tagMap: {} };
  return PlacesAdapter.runAsyncQuery(stmt, processReadBookmarksQueryRow, results);
}
...
function readLocalBookmarks() {
  ...
  then(function (results) {
    let idToGUIDMap = results.idToGUIDMap,
        ...
        tagMap = results.tagMap;
    var bookmarkItems = [];
    Object.keys(itemMap).forEach(function (guid) {
      let itemInfo = itemMap[guid],
          parent = kSpecialIds.specialGUIDForId(itemInfo.parent) || idToGUIDMap[itemInfo.parent];
      // re-write local ids to global ones
      itemInfo.localId = itemInfo.id;
      itemInfo.localParent = itemInfo.parent;
      itemInfo.id = guid;
      itemInfo.parent = parent;
      if (parent in tagMap) { // this is a taggedUrl item

      } else {
        bookmarkItems.push(new BookmarksItem(itemInfo));
      }
    });
    return bookmarkItems.map(function (bi) { return bi.toJSON(); });
...
function topoSortFolders(folders) {
  ...
  folders.forEach(function (folder) {
    edges.push([ folder.parent, folder.id ]);
...
function updateItem(bookmarksItem) {
  L.log("update item", bookmarksItem);
}
...
function createFolder(bookmarksItem, localParentId) {
  L.log("about to create folder", localParentId);
...
function createItem(bookmarksItem) {
  L.log("creating item for", bookmarksItem);
  let localParentId = guidToIdMap[bookmarksItem.parent];
  let promise;
  if (localParentId) promise = findExistingItemIds([bookmarksItem.parent]);
  else promise = resolve(null);
  return promise.
  then(function () {
    localParentId = guidToIdMap[bookmarksItem.parent];
    if (bookmarksItem.isFolder()) {
      return createFolder(bookmarksItem, localParentId);
    }
    else if (bookmarksItem.isBookmark()) {
      return createBookmark(bookmarksItem, localParentId);
    }
    else if (bookmarksItem.isLivemark()) {
      // TODO
    }
    else if (bookmarksItem.isQuery()) {
      // TODO
    }
    else if (bookmarksItem.isSeparator()) {
      // TODO
    }
  }).
  then(function (localId) {
    ...
    // remote guid
    return setItemIdForGuid(localId, bookmarksItem.id).
    then(function (result) {
      L.log("created item", bookmarksItem, localParentId, localId);
      return null;
    });
  });
}

function findExistingItemIds(itemGuids) {
  var stmt = GET_IDS_FOR_GUIDS_STMT;
  var guids = Object.keys(itemGuids).map(function(guid) { return "'"+guid+"'";}).join(',');
  var params = stmt.newBindingParamsArray();
  let bp = params.newBindingParams();
  bp.bindByName('guids', guids);
  params.addParams(bp);
  stmt.bindParameters(params);
  return PlacesAdapter.runAsyncQuery(stmt, function (row, results) {
    let id = row.getResultByName('b_id'),
        guid = row.getResultByName('b_guid');
    guidToIdMap[guid] = id;
  }, {});
}

function setItemIdForGuid(localId, guid) {
  ...
  then(function() {
    ...
    return { localId: guidToIdMap[guid], guid: guid };
  });
}

function updateItems(bookmarksItems) {
  var itemMap = {};
  bookmarksItems.forEach(function (item) {
    itemMap[item.id] = item;
    if (!(item.parent in itemMap) && !kSpecialIds.isSpecialGUID(item.parent)) {
      itemMap[item.parent] = null;
    }
  });
  var itemGuids = Object.keys(itemMap);
  return findExistingItemIds(itemGuids).
  then(function () {
    var processGuid = function(guid) {
      if (guid in guidToIdMap) {
        return updateItem(itemMap[guid]);
      }
      else {
        return createItem(itemMap[guid]);
      }
    };

    let result = resolve(null);
    Object.keys(itemMap).forEach(function (guid) {
      result = result.then(function() { return processGuid(guid); });
    });
    return result;
  });
}

function updateLocalBookmarks(bookmarksItems) {
  let sortedFolders = topoSortFolders(bookmarksItems.filter(function (item) { return item.isFolder(); }));
  let notFolders = bookmarksItems.filter(function (item) { return !item.isFolder(); });
  return updateItems(sortedFolders.concat(notFolders)).
  then(function () {
    L.log("Done updating localBookmarks");
    return null;
  });
}
...
//   if (bookmarkInfo.itemId) bookmark.id = getBookmarkUUIDForItemId(bookmarkInfo.itemId);
//   else bookmark.id = bookmarkInfo.id;
