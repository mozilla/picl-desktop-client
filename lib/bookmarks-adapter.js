...
function processBookmarksReadQueryRow(row, results) {
  var oneResult,
  ...
  if (!kSpecialIds.isSpecialGUID(guid)) {
    ...
    if (oneResult.parent === kSpecialIds.tags) {
...
function readRawAllBookmarks() {
  ...
  return PlacesAdapter.runAsyncQuery(stmt, processBookmarksReadQueryRow, results);
}
